...
if ( ( ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_home() ) {
		...
		if ( is_search() ) {
			...
			$output_html .= $before . '<a href="' . get_post_type_archive_link( 'product' ) . '">' . $_name . '</a>' . $delimiter . esc_html__( 'Search results for &ldquo;', 'zilom' ) . get_search_query() . '&rdquo;' . $after;
		...
		$output_html .= $before . esc_html__( 'Search results for &ldquo;', 'zilom' ) . get_search_query() . '&rdquo;' . $after;
