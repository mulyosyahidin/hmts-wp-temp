...
================================================== */
if ( ! function_exists( 'zilom_base_sidebar' ) ) {
	 function zilom_base_sidebar() {
		  // VARIABLES
		  global $post;

		  // DEFAULT SIDEBAR CONFIG
		  $default_sidebar_config = zilom_get_option('default_sidebar_config', ''); 
		  $default_left_sidebar   = zilom_get_option('default_left_sidebar', ''); 
		  $default_right_sidebar  = zilom_get_option('default_right_sidebar', '');
		  ...
		  if ( is_search() || is_archive() || is_author() || is_category() || is_home() ) {
			 $default_sidebar_config = zilom_get_option('archive_sidebar_config', ''); 
			 $default_left_sidebar   = zilom_get_option('archive_sidebar_left', '');
			 $default_right_sidebar  = zilom_get_option('archive_sidebar_right', ''); 
		  }
		  ...
		  if ( $post && is_singular() ) {
			 $sidebar_config = get_post_meta( $post->ID, 'zilom_sidebar_config', true );
			 $left_sidebar   = get_post_meta( $post->ID, 'zilom_left_sidebar', true );
			 $right_sidebar  = get_post_meta( $post->ID, 'zilom_right_sidebar', true );
		  }
...
function zilom_sidebar_global($sidebar_layout_config, $sidebar_left_config, $sidebar_right_config){

	$left_sidebar          = array('active' => false);
	$right_sidebar         = array('active' => false);
	$main_content          = array('class' => 'clearfix');
	...
	if ( $sidebar_layout_config == "left-sidebar" && is_active_sidebar( $sidebar_left_config )) {
		...
		$main_content = array('class' => 'col-xl-8 col-lg-8 col-md-12 col-sm-12 col-xs-12 order-xl-2 order-lg-2 order-md-1 order-sm-1 order-xs-1');
		...
		$main_content = array('class' => 'col-xl-6 col-lg-6 col-md-12 col-sm-12 col-xs-12 order-xl-2 order-lg-2 order-md-1 order-sm-1 order-xs-1');
	}
  return array(
...
function zilom_base_layout($template){
  $post_id = zilom_id();
  $sidebar_var           = zilom_base_sidebar();
  ...
  $sidebar_config = zilom_sidebar_global($sidebar_var['config'], $sidebar_var['left'], $sidebar_var['right']);
  extract($sidebar_config);
  
  $page_full_width = get_post_meta($post_id, 'zilom_page_full_width', true);
  ...
  ?>
	 ...
	 <div class="content-page-wrap">
		<?php do_action( 'zilom_before_main_content' ); ?>
		 <div class="main-page-content base-layout <?php echo esc_attr($page_wrap_class); ?>">
			 
				<div class="content-page <?php echo esc_attr($main_content_config['class']); ?>">
				  <div class="content-page-inner">   
						<?php zilom_get_template($template) ?>
				  </div>    
				</div>      
		  ...
		  <?php do_action( 'zilom_after_main_content' ); ?>
		</div>
	 </div>
...
if ( ! function_exists( 'zilom_get_template' ) ) {
	 function zilom_get_template( $template ) {
