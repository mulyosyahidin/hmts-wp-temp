<?php
class Zilom_Listing_Comment_BE extends Zilom_Listing_Comment{
	...
	public function output_html( $comment, $box ) {
		...
		<table class="form-table">
			...
			<tr><td colspan="2"><h3><?php echo esc_html__('Category review was deleted', 'zilom-themer') ?></h3><td></tr>
			<?php foreach ( $categories_star as $cat ) :
				...
				<tr>
				  	...
				  	<td> 
					  	<select name="lt_review[<?php echo $cat ?>]">
							...
							<?php endfor; ?>
					 		<option value="delete"><?php echo esc_html__('Delete', 'zilom-themer') ?></option>
					 	</select>
			 ...
			 <?php wp_nonce_field( 'zilom_lt_save_data', 'zilom_lt_meta_nonce' ); ?>
		</table>
	<?php
	}
	...
	public function save_comment( $comment_id, $data ) {
		...
		if ( empty( $_POST['zilom_lt_meta_nonce'] ) || ! wp_verify_nonce( $_POST['zilom_lt_meta_nonce'], 'zilom_lt_save_data' ) ) {
			return $comment_id;
		}
